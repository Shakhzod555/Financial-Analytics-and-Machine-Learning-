...
% Off-white background colour matching the UCL logo PNG so the logo
% blends in without a visible rectangle. Used only on the cover page.
\definecolor{coverbg}{RGB}{250,250,252}
...
% =====================================================================
\begin{titlepage}
\pagecolor{coverbg}% tint cover page background to match logo
\centering
\vspace*{2cm}
...
\vspace{1.2em}
{\LARGE Replication of Christensen, Siggaard \& Veliyev\\(2023, JFE)\par}
\vspace{3cm}

\begin{tabular}{rl}
\textbf{Student ID:}      & 25130184 \\[0.35em]
\textbf{Name:}            & Shakhzod Makhmudov \\[0.35em]
\textbf{Module:}          & IFTE0004 \\[0.35em]
\textbf{Code repository:} & \texttt{<URL to be added>} \\
\end{tabular}

\vfill
{\normalsize Financial Analytics and Machine Learning --- Coursework Report}
\vspace{1cm}
\end{titlepage}
...
and MCS retention sit in the appendix (Tables~4--7), with the full
...
Descriptive statistics are in Table~1. Figure~\ref{fig:rv} plots
...
QLIKE (Table~3) is treated as the operationally relevant ranking in
...
numerical results sit in Table~2; Figure~\ref{fig:relmse}
...
contest (Tables~4--7, paper-faithful) but fixed-window in the
8-model headline contest (Tables~1--3 and headline figures) for compute
...
long horizons (Table~6), replicating Finding~(ii) for regularised
...
headline is recovered (point~(b); Table~6). I expected the gain to
...
(Table~6). (4)~Rolling refit narrows AMZN by $\sim13\,\%$ but the
...
GBoost; Table~6), plausibly because 3 stocks concentrate
...
- 1$). Table~3 re-evaluates under QLIKE. LogHAR remains best at
...
with Table~3b, the picture sharpens: under proxy-aware losses the
...
outlier protection. The AMZN-driver evidence sits in Table~3b's
...
$=1.48$ vs $\mathrm{NN}_3^{10}=1.04$; Table~6), supporting the
